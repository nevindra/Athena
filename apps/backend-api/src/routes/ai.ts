...
import { handleChatRequest, handleStreamChatRequest, handleGetModelsRequest } from "../controllers/aiController";
...
export const aiRoutes = new Elysia({ prefix: "/ai" })
  ...
  .post("/chat/stream", async ({ body }) => {
    try {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: error instanceof Error ? error.message : "Failed to process stream chat request",
        }),
        {
          status: 500,
  ...
  .get("/models", async ({ query }) => {
    try {
      const models = await handleGetModelsRequest(query.userId, query.configurationId);
      return {
        success: true,
        data: {
          models,
