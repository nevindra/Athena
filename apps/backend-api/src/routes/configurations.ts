...
export const configurationsRoutes = new Elysia({ prefix: "/configurations" })
  ...
  .post("/", async ({ body, query }) => {
    ...
    if (!validateSettingsForProvider(validation.data.provider, validation.data.settings)) {
      return {
        success: false,
    ...
    await configurationController.ensureUser(userId);
    return configurationController.createConfiguration(userId, validation.data);
  }, {
    body: t.Object({
      name: t.String(),
      provider: t.Union([t.Literal("gemini"), t.Literal("ollama"), t.Literal("http-api")]),
      settings: t.Any(),
  ...
  .put("/:id", async ({ params, body, query }) => {
    ...
    return configurationController.updateConfiguration(userId, configId, validation.data);
  }, {
    params: t.Object({
      id: t.String(),
    }),
