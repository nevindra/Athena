...
export const systemPromptCategorySchema = z.enum([
  "Structured Output",
  "Topic Specific",
  "Custom",
]);
...
export const createSystemPromptRequestSchema = z
  .object({
    title: z
      .string()
  ...
  .refine(
    (data) => {
      ...
      if (data.category === "Structured Output") {
        return (
          data.jsonSchema && data.jsonSchema.length > 0 && data.jsonDescription
        );
      }
      return true;
    },
    {
      message:
        "Structured Output category requires JSON schema and description",
      path: ["jsonSchema"],
    }
  );
...
export const systemPromptIdSchema = z
  .string()
  .min(1, "System prompt ID is required");

// Type exports
export type CreateSystemPromptRequest = z.infer<
  typeof createSystemPromptRequestSchema
>;
export type UpdateSystemPromptRequest = z.infer<
  typeof updateSystemPromptRequestSchema
>;
