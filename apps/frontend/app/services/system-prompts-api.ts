...
export const systemPromptsApi = {
  ...
  async getSystemPrompts(): Promise<SystemPrompt[]> {
    return makeApiCall(() =>
      apiClient.get(`system-prompts?userId=${DEMO_USER_ID}`).json<ApiResponse<SystemPrompt[]>>()
    );
  },
  ...
  async getSystemPrompt(promptId: string): Promise<SystemPrompt> {
    return makeApiCall(() =>
      apiClient.get(`system-prompts/${promptId}?userId=${DEMO_USER_ID}`).json<ApiResponse<SystemPrompt>>()
    );
  },
  ...
  async createSystemPrompt(data: CreateSystemPromptRequest): Promise<SystemPrompt> {
    return makeApiCall(() =>
      apiClient.post(`system-prompts?userId=${DEMO_USER_ID}`, {
        json: data
  ...
  async updateSystemPrompt(
    ...
    return makeApiCall(() =>
      apiClient.put(`system-prompts/${promptId}?userId=${DEMO_USER_ID}`, {
        json: data
      }).json<ApiResponse<SystemPrompt>>()
    );
  },
  ...
  async deleteSystemPrompt(promptId: string): Promise<void> {
    await makeApiCall(() =>
      apiClient.delete(`system-prompts/${promptId}?userId=${DEMO_USER_ID}`).json<ApiResponse<null>>()
    );
  },
};
